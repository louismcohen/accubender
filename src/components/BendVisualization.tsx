			...
			>
				...
				>
					...
					<g stroke="#f0f0f0" strokeWidth="0.5">
						...
						{Array.from({ length: 11 }).map((_, i) => (
							<line key={`v-${i}`} x1={i * 30} y1="0" x2={i * 30} y2="500" />
						))}
					</g>

					{/* Starting point */}
					<circle cx="250" cy="100" r="4" fill="#888" />
					...
					<g transform="translate(20, 40)">
						<line
							...
							strokeDasharray="4 2"
						/>
						<text x="30" y="5" className="text-xs fill-current">
							Expected
						</text>
						...
						<line
							...
							strokeWidth="4"
						/>
						<text x="30" y="25" className="text-xs fill-current">
							Actual
						</text>
					</g>
					...
					{failedInspections.map((failure) => {
						...
						const bendIndex = bends.findIndex(
							(bend) => bend.position === failure.bendPosition,
						);
					...
					<ul className="list-disc list-inside mt-1">
						{failedInspections.map((failure) => (
							<li key={failure.bendPosition}>
								Bend {failure.bendPosition}: Expected {failure.expected}°,
								measured {failure.actual.toFixed(1)}° (deviation:{" "}
								{Math.abs(failure.deviation).toFixed(1)}°)
							</li>
						))}
					</ul>
				</div>
