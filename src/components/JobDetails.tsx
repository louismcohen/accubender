      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
            {job.bends.map((bend) => (
              <div key={bend.position} className="flex items-center justify-between p-2 bg-muted rounded-md">
                <div>
                  <span className="font-medium">Bend {bend.position}</span>
                  {bend.modified && (
                    <Badge variant="outline" className="ml-2 text-xs bg-blue-100 border-blue-300 text-blue-800">
                      Modified
                    </Badge>
                  )}
                </div>
                <div className="text-right">
                  ...
                  <div className="text-xs text-muted-foreground">{bend.length}mm</div>
                </div>
              </div>
            ))}
          </div>
