...
export default function JobPending() {
  ...
  const [machineState, setMachineState] = useState<MachineState>(MachineState.CONNECTING);
  ...
  useEffect(() => {
    ...
    } else if (machineState === MachineState.READY && tubeDetected) {
      ...
      <div className="lg:col-span-3 space-y-6">
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className={`p-6 rounded-lg border-2 text-center ${getMachineStatusClass(machineState)}`}>
              <div className="flex justify-center items-center space-x-3 mb-4">
                ...
                <h2 className="text-xl font-semibold">{getMachineStatusText(machineState)}</h2>
              </div>
              
              {(machineState === MachineState.CONNECTING || machineState === MachineState.LOADING) && (
                <div className="space-y-2 max-w-md mx-auto">
                  <Progress value={progress} />
                  <p>{machineState === MachineState.CONNECTING ? 'Connecting to bending machine...' : 'Loading bend specifications...'}</p>
                </div>
              )}
              
              {machineState === MachineState.READY && (
                <div className="space-y-4">
                  <p className="text-lg">Machine is ready and waiting for tube insertion</p>
              ...
              {machineState === MachineState.ERROR && (
                <div className="space-y-4">
                  <p>Failed to connect to bending machine. Please check machine power and network connection.</p>
            ...
            {machineState === MachineState.READY && (
              <div className="border rounded-lg p-4">
                ...
                <ol className="list-decimal list-inside space-y-2">
                  <li className="text-muted-foreground">Insert the {job.material} tube (diameter: {job.diameter}mm) into the machine</li>
                  <li className="text-muted-foreground">Ensure tube is properly aligned with markers</li>
                  <li className="text-muted-foreground">Bending will begin automatically once tube is detected and secured</li>
                </ol>
              </div>
            )}
          </CardContent>
