...
export default function InspectionPending() {
  ...
  const [inspectionState, setInspectionState] = useState<InspectionState>(InspectionState.WAITING);
        ...
        timer = setTimeout(() => {
          if (failInspection) {
            toast({
              ...
              description: "Part has out-of-spec bends. See details for more information.",
              variant: "destructive",
            });
          } else {
            toast({
  ...
  }, [job, inspectionState, tubeDetected, doorClosed, navigate, toast, failInspection]);
      ...
      <div className="lg:col-span-3 space-y-6">
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className={`p-6 rounded-lg border-2 text-center ${getInspectionStatusClass(inspectionState)}`}>
              ...
              {inspectionState === InspectionState.WAITING && (
                <div className="space-y-4">
                  <p className="text-lg">Please insert the tube into the inspection chamber</p>
                  <div className="flex justify-center space-x-3">
                    <Badge variant="outline" className={tubeDetected ? 'bg-green-100 text-green-800 border-green-300' : ''}>
                      {tubeDetected ? 'Tube Detected' : 'No Tube Detected'}
                    </Badge>
                    <Badge variant="outline" className={doorClosed ? 'bg-green-100 text-green-800 border-green-300' : ''}>
              ...
              {inspectionState === InspectionState.COMPLETE && (
                <div className="space-y-4">
                  <p className="text-lg">Inspection process completed. Generating results...</p>
            ...
            {inspectionState === InspectionState.WAITING && (
              <div className="border rounded-lg p-4">
                ...
                <ol className="list-decimal list-inside space-y-2">
                  <li className="text-muted-foreground">Insert the bent tube into the inspection fixture</li>
                  <li className="text-muted-foreground">Ensure tube is seated properly on all contact points</li>
                  <li className="text-muted-foreground">Close the inspection chamber door</li>
                  <li className="text-muted-foreground">Scanning will begin automatically</li>
                </ol>
              </div>
            )}
            
            {(inspectionState === InspectionState.SCANNING || inspectionState === InspectionState.ANALYZING) && (
          ...
          <CardFooter>
            <Button variant="outline" disabled={inspectionState !== InspectionState.WAITING} onClick={() => navigate('/dashboard')}>
              Back to Job Queue
            </Button>
          </CardFooter>
        </Card>
