...
import { mockReworkSuggestions } from '@/data/mockJobs';
...
export default function ReworkJob() {
  ...
  const [suggestedAdjustments, setSuggestedAdjustments] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    ...
    setSuggestedAdjustments(mockReworkSuggestions as Record<string, any>);
    setSuggestedRationale(mockReworkSuggestions.rationale as string);
  ...
  const applySuggestions = () => {
    ...
    toast({
      ...
      description: "AI recommendations have been applied to the adjustment values.",
    });
  };
  ...
  const generateNewSuggestions = () => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: "ML model has analyzed the inspection results and provided new recommendations.",
      });
  ...
  const failedResults = job.inspectionResults?.filter(result => !result.pass) || [];
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mb-3">
                    The following bends did not meet the required specifications:
                  </p>
                  
                  <div className="space-y-2">
                    {failedResults.map(result => (
                      <div key={result.bendPosition} className="flex items-center justify-between p-2 bg-white rounded border">
                        <div>
                          <span className="font-medium">Bend {result.bendPosition}:</span>
                          <span className="ml-2 text-sm">
                            Expected {result.expected}°, Actual {result.actual.toFixed(1)}°
                          </span>
                        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 border rounded-lg bg-gray-50">
                <div className="mb-4">
                  ...
                  <p className="text-sm text-muted-foreground">{suggestedRationale}</p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {Object.keys(suggestedAdjustments).length > 0 ? (
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {Object.entries(suggestedAdjustments)
                        ...
                        .map(([bendKey, value]) => {
                          ...
                          return (
                            <div key={bendKey} className="flex flex-col p-2 bg-white border rounded-md">
                              <span className="text-sm font-medium">Bend {bendPosition}</span>
                              <span className={`text-sm ${value < 0 ? 'text-blue-600' : 'text-red-600'}`}>
                                {value > 0 && '+'}
                                {value.toFixed(1)}°
                              </span>
                            </div>
                    ...
                    <p className="text-sm text-muted-foreground italic">No adjustments suggested</p>
                  )}
                  ...
                  <Button 
                    ...
                    disabled={Object.keys(suggestedAdjustments).filter(key => key !== 'rationale').length === 0}
                  >
                    Apply Suggestions
                  </Button>
                </div>
                  ...
                  return (
                    <div key={bendKey} className="flex items-center space-x-4">
                      <div className="w-24">
                        ...
                        <div className="text-sm text-muted-foreground">{bend.angle}°</div>
                      </div>
                      
                      {isEditing ? (
                        <div className="flex-1">
                          <Label htmlFor={bendKey} className="sr-only">Adjustment for Bend {bend.position}</Label>
                          <div className="relative">
                            <Input
                              ...
                              onChange={(e) => updateAdjustment(bendKey, e.target.value)}
                              className="pl-8"
                              placeholder="0.0"
                            />
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">Δ</span>
                          </div>
                        </div>
                      ) : (
                        <div className="flex-1">
                              ...
                              <TooltipTrigger asChild>
                                <div 
                                  className={`
                                    ...
                                    ${hasAdjustment 
                                      ? (adjustment < 0 ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800')
                      ...
                      <div className="w-24 text-right">
                        <div className="font-medium">
                          {hasAdjustment ? (bend.angle + adjustment).toFixed(1) : bend.angle}°
                        </div>
