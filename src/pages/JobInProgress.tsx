      ...
      <div className="lg:col-span-3 space-y-6">
        <Card>
          ...
          <CardContent className="space-y-6">
            {allBendsComplete ? (
              <div className="p-8 border-2 border-green-500 bg-green-50 rounded-lg text-center">
                ...
                <h2 className="text-2xl font-semibold text-green-700 mb-2">Bending Complete</h2>
                <p className="text-green-700 mb-6">All bends have been completed successfully.</p>
                <Button 
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 bg-muted rounded-lg">
                          ...
                          <p className="text-xl font-semibold">{currentBend.position}</p>
                        </div>
                        <div className="p-4 bg-muted rounded-lg">
                          <p className="text-sm text-muted-foreground mb-1">Angle</p>
                          <p className="text-xl font-semibold">{currentBend.angle}°</p>
                        </div>
                        <div className="p-4 bg-muted rounded-lg">
                          <p className="text-sm text-muted-foreground mb-1">Length</p>
                          <p className="text-xl font-semibold">{currentBend.length}mm</p>
                        </div>
                        <div className="p-4 bg-muted rounded-lg">
                          <p className="text-sm text-muted-foreground mb-1">Type</p>
                          <p className="text-xl font-semibold capitalize">{currentBend.type}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          {bendComplete ? (
                            <p className="text-green-600 font-medium">Bend complete</p>
                          ) : (
                            <p className="text-blue-600 font-medium">Bending in progress...</p>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center mt-1">
                      {bendProgress > 90 ? (
                        <>
                          ...
                          <span className="text-sm text-green-600">Good bend quality</span>
                        </>
                      ) : bendProgress > 50 ? (
                        <>
                          <AlertTriangle className="h-4 w-4 text-amber-600 mr-1" />
                          <span className="text-sm text-amber-600">Verifying bend quality</span>
                        </>
                      ) : (
                        <>
          ...
          <CardFooter className="justify-between">
            <Button variant="outline" disabled={!allBendsComplete} onClick={() => navigate('/dashboard')}>
              Back to Job Queue
            </Button>
