...
export function JobsProvider({ children }: { children: ReactNode }) {
	...
	const completeJob = (jobId: string, success: boolean) => {
		...
		if (success) {
			toast({
				title: "Job Completed",
				description:
					"The part has passed inspection and is ready for delivery.",
				variant: "default",
			});
		} else {
			toast({
				...
				description:
					"The part requires rework. See details for suggested adjustments.",
				variant: "destructive",
			});
		}
	};
	...
	const reworkJob = (jobId: string, adjustments: Record<string, number>) => {
		...
		toast({
			...
			description:
				"Adjustments have been applied to the bend file. Ready to rebend.",
		});
	};
